 ...
 *
 * Sheetet förväntas ha kolumnerna: Tidstämpel | Datum | Meter | Anteckning (rad 1 = rubriker)
 */

function doGet(e) {
  try {
    // Om action=add, spara ett nytt pass
    if (e.parameter && e.parameter.action === 'add') {
      return handleAdd(e.parameter);
    }

    // Annars returnera all data
    return getAllRows();
...
function handleAdd(params) {
  ...
  sheet.appendRow([new Date(), date, meters, note]);

  return jsonResponse({ success: true, date: date, meters: meters });
}
